...
<body>
    <?php
        ...
        $sql=  "SELECT PD.pet_det_ID, PD.pet_name,AR.pet_det_ID
                FROM pet_details PD 
                LEFT JOIN pet_attributes AR 
                ON AR.pet_det_ID = PD.pet_det_ID 
                WHERE user_id='17' AND AR.pet_det_ID IS NULL";
        $res=$dbh->query($sql);
                    ...
                    if(isset($_POST['submit']))
                        {
                            ...
                            $food=$_POST['food'];
                            $today=date("Y-m-d");
                            $petattrid =$_SESSION['pet_attr_ID'];
                            
                            $qry=$dbh->prepare("INSERT INTO  pet_attributes VALUES('$petattrid','$petdetid',
                                '$height','$weight','$lenght','$color','$vaccine','$nutrient','$medicine',
                                '$heatperiod','$food','$today')");
                            $excu=$qry->execute($val);
                            if($excu)
                            {
                             echo "<script>alert('Data inserted');window.location='features.php';</script> ";
                            }
                            else
                            {
